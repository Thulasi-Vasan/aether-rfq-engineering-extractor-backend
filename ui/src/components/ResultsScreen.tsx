...
export default function ResultsScreen({ result, pdfUrl, onReset }: Props) {
  const [openOps, setOpenOps] = useState<Set<number>>(
    () => new Set([result.operations[0]?.opn_no].filter(Boolean) as number[]),
  );
    ...
    <div className="min-h-[calc(100vh-56px)] bg-[#F8FAFC] px-4 py-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        >
          ...
          <button
            onClick={onReset}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-border bg-white px-5 text-sm font-semibold text-navy-700 shadow-sm transition-colors hover:bg-bg-surface"
          >
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onToggle}
        className="flex w-full items-start gap-4 px-4 py-4 text-left transition-colors hover:bg-bg-surface/70"
      >
        <span className="mt-0.5 flex h-9 w-9 flex-none items-center justify-center rounded-lg bg-navy-800 text-xs font-bold text-white">
          {op.opn_no}
        </span>
  ...
  useEffect(() => {
    ...
    async function renderPage() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Unable to read uploaded PDF (${response.status}).`);
        }
    ...
    <div
      className={cn(
        ...
        type === "region"
          ...
          : "border border-white bg-white/25",
      )}
      style={style}
    />
  );
...
function Fact({
  ...
  return (
    <div className={compact ? "" : "rounded-lg border border-border bg-bg-surface p-3"}>
      <p className="text-[11px] font-semibold uppercase tracking-wider text-text-muted">
        {label}
      </p>
      <p className="mt-1 break-words text-sm font-medium leading-5 text-navy-900">
        {value || "N/A"}
      </p>
    </div>
  );
...
function Metric({
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-3">
      ...
      <p
        className={cn(
          "break-words font-mono text-sm font-semibold text-navy-900",
          mock && "italic text-text-muted",
          strong && "text-base",
        )}
      >
        {value}
...
function GeometryItem({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className="break-words font-mono text-sm font-semibold text-navy-900">
        {value}
      </span>
    </div>
  );
